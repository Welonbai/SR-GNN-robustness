...
def test_creat_config_parses_defaults_from_yaml() -> None:
    ...
    assert creat.variant == "v1"
    assert creat.dpp_reward_weight == 0.0


def test_creat_v2_config_parses_and_changes_identity() -> None:
    config = load_config(
        "attack/configs/diginetica_valbest_attack_create_copy_source_popular_sample.yaml"
    )
    creat = config.attack.creat_additive_sbr
    assert creat is not None
    assert creat.variant == "v2"
    assert creat.dpp_score_mode == "bounded_determinant"
    assert creat.dpp_reward_weight == 0.0
    assert config.targets.mode == "sampled"
    assert config.targets.bucket == "popular"
    assert config.targets.count == 10
    changed = replace(
        config,
        attack=replace(
            config.attack,
            creat_additive_sbr=replace(creat, pattern_reward_weight=0.2),
        ),
    )
...
def test_creat_config_rejects_unsupported_v1_values() -> None:
    ...
    with pytest.raises(ValueError, match="attack_epochs"):
        CreatAdditiveSBRConfig(enabled=True, variant="v2", attack_epochs=0)
    with pytest.raises(ValueError, match="local_window_size"):
        CreatAdditiveSBRConfig(variant="v2", local_window_size=2)
    with pytest.raises(ValueError, match="dpp_score_mode"):
        CreatAdditiveSBRConfig(variant="v2", dpp_score_mode="shifted")
