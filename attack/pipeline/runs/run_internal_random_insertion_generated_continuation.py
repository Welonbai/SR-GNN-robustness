...
DEFAULT_INTERNAL_RANDOM_INSERTION_GENERATED_CONTINUATION_CONFIG_PATH = (
    "attack/configs/"
    "diginetica_valbest_attack_internal_random_insertion_generated_continuation_"
    "nonzero_when_possible_ratio1_srgnn_partial4.yaml"
)


def run_internal_random_insertion_generated_continuation_nonzero(
    config: Config,
    config_path: str | Path | None = None,
) -> dict[str, object]:
    if not config.data.poison_train_only:
        raise ValueError(
            "Internal-Random-Insertion-Generated-Continuation-NZ expects "
    ...
    def build_poisoned(target_item: int) -> TargetPoisonOutput:
        ...
        slot_stats_payload = build_slot_stats_payload(
            ...
            target_item=int(target_item),
            note=(
                "Internal-Random-Insertion-Generated-Continuation-NZ samples "
                "internal non-tail insertion slots, discards the original suffix "
                "after target, and regenerates a suffix of the same length."
            ),
        )
        slot_stats_path = target_root / "internal_insertion_generated_continuation_slot_stats.json"
        ...
        metadata_path = target_root / "internal_random_insertion_generated_continuation_metadata.json"
        ...
        return TargetPoisonOutput(
            ...
            metadata={
                "internal_insertion_generated_continuation_slot_stats_path": str(
                    slot_stats_path
                ),
                "internal_random_insertion_generated_continuation_metadata_path": str(
                    metadata_path
                ),
...
def build_internal_random_insertion_generated_continuation_metadata(
    *,
    config: Config,
    run_type: str,
    operation: str,
    ...
    insertion_results: Sequence[InternalRandomInsertionGeneratedContinuationResult],
    ...
    inserted_sessions = [
        result.inserted_session_before_generation for result in insertion_results
    ]
    generated_suffixes = [result.generated_suffix for result in insertion_results]
    original_suffixes = [result.original_suffix for result in insertion_results]
    ...
    target_position_counts = Counter(
        int(result.target_position) for result in insertion_results
    )
    generated_contains_target_counts = [
        int(result.generated_suffix_contains_target_count)
    ...
    after_insertion_counts = [
        int(result.target_occurrence_count_after_insertion)
        for result in insertion_results
    ]
    generated_first_items = [
        int(result.generated_suffix[0])
        for result in insertion_results
        if result.generated_suffix
    ]
    ...
    return {
        ...
        "suffix_strategy": str(suffix_strategy),
        "target_item": target,
        ...
        "generation_smoothing": "min_max_smooth",
        "generated_suffix_source": "poison_model_score_session_autoregressive",
        "suffix_length_strategy": "preserve_inserted_suffix_length",
        "original_suffix_preserved": False,
        "all_sessions_regenerate_suffix": True,
        ...
        "pure_generated_mode_uses_shared_rng_with_generated_continuation": True,
        ...
        "slot0_excluded": True,
        "target_position_counts": _distribution_from_counter(target_position_counts),
        "target_position_ratios": _ratio_dict(target_position_counts),
        ...
        "every_final_target_has_left_neighbor": bool(
            all(result.target_position > 0 for result in insertion_results)
        ),
        "every_final_target_has_right_neighbor": bool(
            all(result.target_position < result.final_length - 1 for result in insertion_results)
        ),
        ...
        "generated_suffix_length_summary": _numeric_summary(
            [int(len(suffix)) for suffix in generated_suffixes]
        ),
        "generated_suffix_replaced_count": total,
        "generated_suffix_replaced_ratio": 1.0 if total else 0.0,
        ...
        "generated_suffix_unique_item_count_summary": _numeric_summary(
            [int(result.generated_suffix_unique_item_count) for result in insertion_results]
        ),
        "generated_suffix_first_item_counts": _distribution_from_counter(
            Counter(generated_first_items)
        ),
        "generated_suffix_first_item_ratios": _ratio_dict(Counter(generated_first_items)),
        ...
        "avoids_truncate_length_shortening": True,
        "avoids_target_tail_confound_from_truncate": True,
        "removes_original_suffix": True,
        "length_preserving_relative_to_inserted_session": True,
        "previews": _generated_continuation_previews(
...
def _validate_internal_insertion_generated_continuation_sessions(
    *,
    template_sessions: Sequence[Sequence[int]],
    final_sessions: Sequence[Sequence[int]],
    results: Sequence[InternalRandomInsertionGeneratedContinuationResult],
    target_item: int,
    ...
    for original, final, result in zip(template_sessions, final_sessions, results):
        ...
        inserted_list = [int(item) for item in result.inserted_session_before_generation]
        generated_suffix = [int(item) for item in result.generated_suffix]
        slot = int(result.insertion_slot)
        if len(original_list) < 2:
            raise RuntimeError("Generated continuation requires template length >= 2.")
        if slot < 1 or slot > len(original_list) - 1:
            raise RuntimeError("Generated continuation selected slot0 or tail slot.")
        expected_inserted = original_list[:slot] + [target] + original_list[slot:]
        if inserted_list != expected_inserted:
            raise RuntimeError("Inserted session before generation is invalid.")
        if [int(item) for item in result.original_suffix] != inserted_list[slot + 1 :]:
        ...
        expected_final = inserted_list[: slot + 1] + generated_suffix
        ...
        if len(final_list) != len(inserted_list):
            raise RuntimeError("Generated continuation changed inserted length.")
        if len(final_list) != len(original_list) + 1:
            raise RuntimeError("Generated continuation final length is not original + 1.")
        if final_list[slot] != target:
            raise RuntimeError("Generated continuation moved the inserted target.")
        if slot <= 0 or slot >= len(final_list) - 1:
            raise RuntimeError("Generated continuation target lacks left or right neighbor.")
        if target not in set(final_list):
            raise RuntimeError("Generated continuation final session is missing target.")
...
def _top_ratios(counter: Counter[int], limit: int, *, total: int) -> dict[str, float]:
    if total <= 0:
        return {str(item): 0.0 for item in _top_counts(counter, limit)}
    return {
        str(item): float(count) / float(total)
        for item, count in sorted(counter.items(), key=lambda pair: (-pair[1], pair[0]))[
            : int(limit)
...
def _generated_continuation_previews(
    *,
    template_sessions: Sequence[Sequence[int]],
    insertion_results: Sequence[InternalRandomInsertionGeneratedContinuationResult],
    limit: int,
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default=DEFAULT_INTERNAL_RANDOM_INSERTION_GENERATED_CONTINUATION_CONFIG_PATH,
        help="Path to YAML config.",
    )
    args = parser.parse_args()

    config = load_config(args.config)
    run_internal_random_insertion_generated_continuation_nonzero(
        config,
