...
def run_internal_random_insertion_nonzero(
    config: Config,
    config_path: str | Path | None = None,
) -> dict[str, object]:
    if not config.data.poison_train_only:
        raise ValueError(
            "Internal-Random-Insertion-NZ expects data.poison_train_only to be true."
        )
    ...
    def build_poisoned(target_item: int) -> TargetPoisonOutput:
        ...
        slot_stats_payload = build_slot_stats_payload(
            ...
            target_item=int(target_item),
            note=(
                "Internal-Random-Insertion-NZ samples zero-based Python insertion "
                "slots from [1, len(session)-1]; slot0 and tail_slot are excluded."
            ),
        )
        slot_stats_path = target_root / "internal_insertion_slot_stats.json"
        ...
        metadata_path = target_root / "internal_random_insertion_metadata.json"
        save_json(metadata, metadata_path)

        return TargetPoisonOutput(
            poisoned=poisoned,
            metadata={
                "internal_insertion_slot_stats_path": str(slot_stats_path),
                "internal_random_insertion_metadata_path": str(metadata_path),
...
def build_internal_random_insertion_metadata(
    *,
    config: Config,
    target_item: int,
    template_sessions: Sequence[Sequence[int]],
    inserted_sessions: Sequence[Sequence[int]],
    insertion_results: Sequence[InternalRandomInsertionResult],
    ...
    length_deltas = [
        int(result.inserted_length) - int(result.original_length)
        for result in insertion_results
    ]
    left_items = [int(result.left_item) for result in insertion_results]
    right_items = [int(result.right_item) for result in insertion_results]
    left_right_pairs = [
    ...
    return {
        ...
        "random_insertion_then_crop_shared_fake_sessions_key": shared_attack_artifact_key(
            config,
        ...
        "left_right_pair_diversity": _pair_diversity_summary(left_right_pairs),
        ...
        "topk_sampling_note": (
            "topk ratio is interpreted as internal insertion slot top-k ratio; "
            "tail slot is excluded."
        ),
        "previews": _internal_insertion_previews(
...
def _validate_internal_inserted_sessions(
    *,
    template_sessions: Sequence[Sequence[int]],
    inserted_sessions: Sequence[Sequence[int]],
    results: Sequence[InternalRandomInsertionResult],
    target_item: int,
) -> None:
    if len(template_sessions) != len(inserted_sessions):
    ...
    for original, inserted, result in zip(template_sessions, inserted_sessions, results):
        ...
        if len(original_list) < 2:
            raise RuntimeError(
                "Internal-Random-Insertion-NZ requires template session length >= 2."
            )
        if slot < 1 or slot > len(original_list) - 1:
            raise RuntimeError("Internal-Random-Insertion-NZ selected slot0 or tail slot.")
        if int(target_item) not in set(inserted_list):
            raise RuntimeError(
                "Internal-Random-Insertion-NZ injected session is missing target item."
            )
        if len(inserted_list) != len(original_list) + 1:
            raise RuntimeError(
                "Internal-Random-Insertion-NZ injected session length delta is not +1."
            )
        expected = original_list[:slot] + [int(target_item)] + original_list[slot:]
        if inserted_list != expected:
            raise RuntimeError(
                "Internal-Random-Insertion-NZ did not preserve original item order."
            )
        if inserted_list[slot - 1] != int(result.left_item):
            raise RuntimeError("Internal insertion left neighbor metadata is invalid.")
        if inserted_list[slot + 1] != int(result.right_item):
            raise RuntimeError("Internal insertion right neighbor metadata is invalid.")
...
def _pair_diversity_summary(pairs: Sequence[tuple[int, int]]) -> dict[str, float]:
    total = int(len(pairs))
    unique = int(len(set(pairs)))
    return {
        "total_pairs": float(total),
        "unique_pair_count": float(unique),
        "unique_pair_ratio": float(unique) / float(total) if total else 0.0,
    }


def _internal_insertion_previews(
    *,
    template_sessions: Sequence[Sequence[int]],
    inserted_sessions: Sequence[Sequence[int]],
    insertion_results: Sequence[InternalRandomInsertionResult],
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        default=DEFAULT_INTERNAL_RANDOM_INSERTION_CONFIG_PATH,
        help="Path to YAML config.",
    )
    args = parser.parse_args()

    config = load_config(args.config)
    run_internal_random_insertion_nonzero(
        config,
